...
var MessageView = Backbone.View.extend({
  ...
  template: _.template('<div class="<-%escName.replace(/\s/g,"_")%>"> \
    <a href=# class="userLink" data-info="<%-escName%>"><%-escName%>:</a><%-escText%></div><br>'),
  render: function(){
...
var MessagesView = Backbone.View.extend({
  ...
  renderMessage: function (message) {
    if (!this.onScreenMessages[message]) {
      var messageView = new MessageView({model:message});
      this.$el.prepend(messageView.render());
      this.onScreenMessages[message] = true;
    }
  }
});
...
var FormView = Backbone.View.extend({
  events: {
    'submit #sendMessage': 'handleSubmit'
  }
  ...
  //listeners on input box data to call render
    //update MessagesView
